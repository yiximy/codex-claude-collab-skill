...
> Claude Code 派发前已做过精简审查（见 `.ai/brief.md` 的复用/跳过清单）。你实现时同样遵循"懒高级开发"的 7 步梯子，**但梯子只在理解问题之后爬**：先读任务和它触碰的代码、端到端追踪真实流程，再决定怎么写。

### 7 步梯子（从第一个命中的台阶停下来）
1. 这个功能真的需要写吗？（YAGNI，投机性需求直接跳过并说明）
2. 代码库里已有？复用（先搜索，别重写几文件之外的现成 helper）
3. 标准库能搞定？用 stdlib
4. 平台原生能力覆盖？用平台
5. 已装依赖能解决？用已装的，不新增依赖
6. 能一行搞定？就一行
7. 才写最小实现

### 规则
- 不写未要求的抽象：单实现的 interface、单产品的 factory、永不变化的 config。
- 不写样板代码，不为"以后"搭脚手架。
- 删除优先于新增；无聊优先于聪明；文件越少越好；最短可用 diff 胜出——但只在理解问题之后。
- 两个 stdlib 方案同大小时，选边界情况正确的那一个（精简是少写代码，不是选更脆弱的算法）。
- 故意砍真实边角（全局锁、O(n²)、朴素启发式）时，用 `ponytail:` 注释标注上限与升级路径。
- Bug 修复 = 修根因不修症状：动某个函数前 grep 它的所有调用点，在共享函数里加一道防护（比在每个调用点各加一道更小）。

### 绝不精简（边界）
输入校验（信任边界）、防数据丢失的错误处理、安全措施、可访问性基础、用户明确要求的内容。非平凡逻辑（分支/循环/解析器/金钱或安全路径）必须留下**一个**可运行的最小验证（`assert` 自检或一个小 `test_*.py`，不引框架、不建 fixtures）；平凡一行不用测（YAGNI 对测试同样适用）。
...
3. 接手方从最新快照 + 工作区现状开始
4. 无 diff 可用 → 交付时**必须**在 `.ai/plan.md` 列出「本次改动文件清单 + 每个文件改了什么」，供 Claude Code 逐文件审查
5. 无版本回滚能力 → 改动高风险文件前，先复制原文件到同一快照目录
### 规则 2：文件驱动协作
